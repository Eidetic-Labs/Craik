...
GEMINI_ADAPTER_VERSION = "0.2.0-preview"
...
class GeminiRunnerAdapter:
    ...
    def metadata(self) -> RunnerMetadata:
        """Return Gemini runner metadata with preview adapter details."""
        base = get_runner_capability_matrix(GEMINI_RUNNER_ID).runner
    ...
    def run(self, request: RunnerAdapterRequest) -> RunnerAdapterResult:
        """Return a normalized Gemini runner result for a compiled prompt request."""
        if request.runner.id != GEMINI_RUNNER_ID:
            raise GeminiRunnerRequestError(
                f"Gemini adapter cannot run request for runner {request.runner.id!r}"
            )
...
def request_from_compiled_prompt(
    ...
    *,
    ...
    gemini = adapter or GeminiRunnerAdapter()
    if compiled.runner_id != GEMINI_RUNNER_ID:
        raise GeminiRunnerRequestError(
            f"compiled prompt runner {compiled.runner_id!r} is not {GEMINI_RUNNER_ID!r}"
        )
    extra_context = context or {}
    return RunnerAdapterRequest(
        id=f"runner_request_{compiled.id}",
        task_id=compiled.task_id,
        runner=gemini.metadata,
...
def _status_from_context(
    ...
    if value not in {"completed", "blocked", "failed", "partial"}:
        raise GeminiRunnerRequestError(f"unsupported Gemini fixture status: {value!r}")
...
def _summary_from_context(context: dict[str, Any], status: RunnerResultStatus) -> str:
    ...
    if status == "completed":
        return "Gemini fixture completed the compiled prompt handoff."
    if status == "blocked":
        reason = _optional_string(context.get("blocked_reason")) or "policy or approval boundary"
        return f"Gemini fixture blocked before execution: {reason}."
    if status == "failed":
        reason = _optional_string(context.get("failure_reason")) or "fixture failure requested"
        return f"Gemini fixture failed before completing execution: {reason}."
    return "Gemini fixture returned a partial result."


def _diagnostics(context: dict[str, Any], live_available: bool) -> list[str]:
    diagnostics = _string_list(context.get("diagnostics"))
    if not live_available:
        diagnostics.append("Gemini live execution unavailable; fixture/prompt-handoff mode used.")
    return diagnostics

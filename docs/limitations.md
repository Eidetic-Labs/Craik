...
- The local SQLite store, project registry, task creation, intent locks, local case file assembler, handoff writer, local memory proposal backend, minimum Stigmem backend compatibility, policy profiles, capability grant checks, central redaction utility, and receipt store exist, but no full policy enforcement engine, runner adapter, or automatic receipt-producing workflow is available yet.
